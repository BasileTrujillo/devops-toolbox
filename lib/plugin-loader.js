...
class PluginLoader {
  ...
  execPlugins(stackName, category) {
        ...
        for (let catListKey = 0; catListKey < catList.length; catListKey++) {
          ...
          for (let filesKey = 0; filesKey < pluginList.length; filesKey++) {
            ...
            if (pluginOpts) {
              ...
              if (pluginOpts.constructor === Object) {
                pluginConfList.push(pluginOpts);
              } else {
                pluginConfList = pluginOpts;
              }

              for (let pluginConfListKey = 0; pluginConfListKey < pluginConfList.length; pluginConfListKey++) {
                pluginConfList[pluginConfListKey].verbose = this.getVerbosity(pluginConfList[pluginConfListKey]);

                if (async) {
                  execList.push(this.execPlugin(
                    pluginsBaseRelPath,
                    pluginBaseName,
                    pluginConfList[pluginConfListKey]
                  ));
                } else {
  ...
  execPlugin(pluginPath, pluginName, options, previousResponses) {
    let Plugin;
