...
class PluginLoader {
  constructor(program, options) {
    ...
    this.pluginAbsBasePath = path.join(__dirname, this.pluginRelBasePath);
    ...
    if (this.options[stackName]) {
      ...
      if (catList.length > 0) {
        ...
        for (let catListKey = 0; catListKey < catList.length; catListKey++) {
          const pluginsBaseAbsPath = path.join(this.pluginAbsBasePath, catList[catListKey]);
          const pluginsBaseRelPath = path.join(this.pluginRelBasePath, catList[catListKey]);
          const files = fs.readdirSync(pluginsBaseAbsPath);

          // For each plugin file inside a category
          for (let filesKey = 0; filesKey < files.length; filesKey++) {
            const pluginBaseName = files[filesKey].replace('.js', '');
  ...
  execPlugin(pluginPath, pluginName, func, options, previousResponses) {
    ...
    try {
      ...
      console.log(path.join(pluginPath, pluginName));
      console.error(error);
      throw new Error(`"${pluginName}" command is not currently supported.`);
    }
